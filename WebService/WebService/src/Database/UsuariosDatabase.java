...
public class UsuariosDatabase implements UsuariosInterface, DatabaseInterface, JSONInterface {
	...
	public String realizarCadastro(String usuario, String senha) {
		try {
			database = conectar();
			String query_existe_usuario = "SELECT * FROM Usuarios WHERE BINARY nome = \"" + usuario + "\"";
			Statement state = database.createStatement();
			ResultSet resultado_existe_usuario = state.executeQuery(query_existe_usuario);
			boolean existeUsuario = resultado_existe_usuario.next();

			JSONObject json = new JSONObject();
			if(existeUsuario == true){
				json.put("STATUS", "ERRO_AO_CADASTRAR");
				json.put("MOTIVO", "USUARIO_JA_EXISTE");	
				...
				if (resultado != 0){
					...
					json.put("STATUS", "ERRO_AO_CADASTRAR");
					json.put("MOTIVO", "ERRO_NO_BANCO_DE_DADOS");	
					resposta = json.toString();
				}
			}
			database.close();
